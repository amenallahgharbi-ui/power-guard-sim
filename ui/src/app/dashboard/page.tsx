...
export default function AlertsPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        // Defensive sort in UI (newest first)
        data.sort((a, b) => (a.ts_min < b.ts_min ? 1 : a.ts_min > b.ts_min ? -1 : 0));
  ...
  const assetOptions = useMemo(() => {
    const s = new Set(alerts.map((a) => a.asset_id));
    return Array.from(s).sort();
  }, [alerts]);

  const filtered = useMemo(() => {
    if (assetFilter === "__ALL__") return alerts;
    return alerts.filter((a) => a.asset_id === assetFilter);
  }, [alerts, assetFilter]);
    ...
    <div className="space-y-4">
      <div className="flex items-end justify-between gap-4">
        ...
        <label className="text-sm">
          <div className="mb-1 text-gray-600">Asset</div>
          <select
